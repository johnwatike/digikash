...
@section('title', __('Merchant Password Reset'))

@section('auth-content')
	<section class="shell">
		<div class="form-panel">
			@include('frontend.layouts.golden.partials._auth_form_top', [
				'backLabel' => __('Back to Merchant Sign In'),
				'backUrl'   => route('merchant.login'),
			])
			...
			@if(session('status'))
				<div class="form-body success is-visible">
					...
					<p>{{ session('status') }}</p>
					<a href="{{ route('merchant.login') }}" class="gold-link gold-link--caps-lg">{{ __('Return to Sign In') }}</a>
				</div>
			@else
				<div class="form-body">
					@if($errors->any())
					...
					<div class="eyebrow form-eyebrow">{{ __('Merchant Recovery') }}</div>
					<h1 class="form-title">
					...
					<form action="{{ route('merchant.password.email') }}" method="POST" novalidate>
						@csrf
						<div class="field {{ $errors->has('email') ? 'has-error' : '' }}">
							<label for="gdk-email">{{ __('Email Address') }}</label>
							<div class="field__wrap has-lead">
								<i class="lead fa-regular fa-envelope"></i>
								<input id="gdk-email" name="email" type="email" placeholder="{{ __('Enter your merchant email') }}" value="{{ old('email') }}" required autocomplete="email" autofocus>
							</div>
							@error('email')
						...
						<div class="form-footer">
							{{ __('Remembered it?') }}
							<a href="{{ route('merchant.login') }}" class="gold-link">{{ __('Return to Sign In') }}</a>
						</div>

						@include('frontend.layouts.golden.partials._auth_portal_switch', ['current' => 'merchant', 'page' => 'forgot'])
					</form>
				</div>
			@endif
		</div>
