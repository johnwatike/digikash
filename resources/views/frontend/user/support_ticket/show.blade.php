...
@section('content')
    <div class="card single-form-card">
        <x-user-feature-header
            ...
            :subtitle="__('Review the full conversation, attachments, and current ticket status.')"
                        ...
                        <div class="d-flex {{ $message->admin_id ? 'justify-content-start' : 'justify-content-end' }} mb-3">
                            @if($message->admin_id)
                                <img src="{{ asset(auth()->user()->avatar_alt) }}"
                                     class="rounded-circle me-2" alt="Admin" width="40" height="40" loading="lazy">
                            @endif
                            ...
                            @if(!$message->admin_id)
                                <img src="{{ asset($ticket->user->avatar_alt) }}"
                                     class="rounded-circle ms-2" alt="User" width="40" height="40" loading="lazy">
                            @endif
                        </div>
                    @endforeach
                </div>
