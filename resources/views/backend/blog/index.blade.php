						...
						<tr>
							<td>
								<img src="{{ asset($blog->thumbnail) }}" alt="{{ $blog->title_text }}"
								     class="img-fluid rounded avatar" width="100" loading="lazy">
							</td>
					...
					@empty
						<tr>
							<td colspan="6">
								@can('blog-create')
									<x-admin-not-found
										:title="__('No blogs found')"
										:message="__('Create a blog post to start publishing content.')"
										icon="fa-newspaper"
										:action-url="route('admin.blog.post.create')"
										:action-label="__('Add New Blog')"
									/>
								@else
									<x-admin-not-found
										:title="__('No blogs found')"
										:message="__('No blog posts are available yet.')"
										icon="fa-newspaper"
									/>
								@endcan
							</td>
