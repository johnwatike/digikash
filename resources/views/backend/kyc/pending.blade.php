					...
					<div class="card-body">
						...
						<div class="table-responsive">
							<table class="table caption-top mb-0">
								...
								@forelse($kycPendingRequests as $submission)
									<tr>
										...
										<td>
											...
											<div class="small text-muted">{{ $submission->updated_at->diffForHumans() }}</div>
										</td>
										@can('kyc-action')
											<td>
								...
								@empty
									<tr>
										<td colspan="{{ auth()->user()?->can('kyc-action') ? 4 : 3 }}">
											<x-admin-not-found
												:title="__('No pending KYC requests found')"
												:message="__('Pending KYC submissions will appear here when users submit verification.')"
												icon="fa-id-card"
											/>
										</td>
									</tr>
