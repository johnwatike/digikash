...
<section id="webhooks" class="content-section">
    ...
    <p>{{ __(setting('site_title').' sends real-time notifications to your specified IPN URL when payment status changes. This ensures you\'re immediately notified of payment completions, failures, and other status updates.') }} <strong>{{ __('Webhooks work identically in both sandbox and production environments.') }}</strong></p>
    ...
    <div class="alert alert-info mb-4">
        <div class="d-flex align-items-center">
            ...
            <div>
                ...
                <p class="mb-0">{{ __('Use the same webhook URL for both sandbox and production. '.setting('site_title').' will include environment context in webhook payloads to help you differentiate between test and live transactions.') }}</p>
            </div>
        </div>
    </div>
    ...
    <table class="api-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ __('Raw HMAC-SHA256 signature for verification') }}</td>
                <td><code>a8b9c2d1e5f3...</code></td>
            </tr>
            <tr>
                <td><code>X-Environment</code></td>
                <td>{{ __('Webhook environment context') }}</td>
...
class {{ setting('site_title') }}WebhookController extends Controller
{
    public function handle(Request $request)
    {
        ...
        if (!$this->verifySignature($request->getContent(), $signature, $secret)) {
            Log::warning('{{ __(setting("site_title")." webhook signature verification failed") }}', [
    ...
    private function getSecretForEnvironment(string $environment): string
    {
        ...
        return $environment === 'sandbox' 
            ? config('{{ strtolower(setting("site_title")) }}.test_webhook_secret')
            : config('{{ strtolower(setting("site_title")) }}.webhook_secret');
    }
...
// {{ __('Webhook handler') }}
app.post('/api/webhooks/{{ strtolower(setting("site_title")) }}', async (req, res) => {
    ...
    if (!verifySignature(JSON.stringify(req.body), signature, secret)) {
        console.warn('{{ __(setting("site_title")." webhook signature verification failed") }}', {
            environment: environment
        });
...
function getSecretForEnvironment(environment) {
    # {{ __('Return test secret for sandbox, live secret for production') }}
    return environment === 'sandbox' 
        ? process.env.{{ strtoupper(setting("site_title")) }}_TEST_WEBHOOK_SECRET
        : process.env.{{ strtoupper(setting("site_title")) }}_WEBHOOK_SECRET;
}
...
def {{ strtolower(setting("site_title")) }}_webhook(request):
    ...
    if not verify_signature(request.body, signature, secret):
        logger.warning('{{ __(setting("site_title")." webhook signature verification failed") }}', extra={
            'environment': environment
        })
...
def get_secret_for_environment(environment):
    ...
    # {{ __('Return test secret for sandbox, live secret for production') }}
    return (settings.{{ strtoupper(setting("site_title")) }}_TEST_WEBHOOK_SECRET 
            if environment == 'sandbox' 
            else settings.{{ strtoupper(setting("site_title")) }}_WEBHOOK_SECRET)
