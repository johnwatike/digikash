...
<section id="testing" class="content-section">
    ...
    <p>{{ __('Test') }} {{ setting('site_title') }} {{ __('API endpoints directly from this documentation. Use the demo credentials below for sandbox testing.') }}</p>
                ...
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body p-3">
                        ...
                        <div class="demo-credential">
                            ...
                            <strong>{{ __('Wallet PIN') }}:</strong>
                            <code class="d-block bg-light p-2 rounded mt-1">123456</code>
                        </div>
            ...
            <div class="col-12">
                <div class="bg-white rounded p-3 border">
                    ...
                    <ul class="mb-0 small text-muted">
                        ...
                        <li><strong>{{ __('Gateway Controls') }}:</strong> {{ __('Hosted checkout respects the gateways configured from Merchant API Config for the selected currency.') }}</li>
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6 mb-3">
                    <label for="testApiSecret" class="form-label">{{ __('API Secret') }}:</label>
                    <input type="password" class="form-control" id="testApiSecret" placeholder="{{ __('Enter the matching API secret') }}">
                    <small class="text-muted">{{ __('Used locally to generate X-Signature. Prefer sandbox secrets in the browser console.') }}</small>
                </div>
            </div>
            ...
            <div id="requestBodySection">
                ...
                <div id="initiatePaymentParams">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="cancelRedirect" class="form-label">{{ __('Cancel Redirect URL') }} <span class="text-danger">*</span></label>
                            <input type="url" class="form-control" id="cancelRedirect" placeholder="https://merchant.com/cancel">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="ipnUrl" class="form-label">{{ __('IPN URL') }} <span class="text-danger">*</span></label>
                            <input type="url" class="form-control" id="ipnUrl" placeholder="https://merchant.com/webhook/{{ strtolower(setting("site_title")) }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            <label for="allowPaymentMethods" class="form-label">{{ __('Allow Payment Methods (Optional)') }}</label>
                            <input type="text" class="form-control" id="allowPaymentMethods" placeholder='stripe,paypal or ["stripe","paypal"]'>
                            <small class="text-muted">{{ __('Limit checkout methods by name. Accepts CSV string or JSON array. Case-insensitive.') }}</small>
                        </div>
                    </div>
                </div>
    ...
    <div class="row mt-4">
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    <p><strong>{{ __('Base URL') }}:</strong> <code>{{ request()->getSchemeAndHttpHost() }}</code></p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            testApiBtn.addEventListener('click', async function() {
                ...
                const apiSecret = document.getElementById('testApiSecret').value.trim();
                
                // Validate required fields
                if (!apiKey || !merchantKey || !apiSecret) {
                    alert('{{ __('Please enter API Key, Merchant Key, and API Secret') }}');
                    return;
                }
                ...
                try {
                    ...
                    if (endpoint === 'initiate-payment') {
                        response = await testInitiatePayment(environment, apiKey, merchantKey, apiSecret);
                    } else {
                        response = await testVerifyPayment(environment, apiKey, merchantKey, apiSecret);
                    }
            ...
            async function buildSignedHeaders(environment, apiKey, merchantKey, apiSecret, method, pathWithQuery, body = '') {
                const timestamp = Math.floor(Date.now() / 1000).toString();
                const signaturePayload = [timestamp, method.toUpperCase(), pathWithQuery, body].join('.');
                const signature = await createHmacSignature(apiSecret, signaturePayload);
            ...
            async function testInitiatePayment(environment, apiKey, merchantKey, apiSecret) {
                ...
                const allowPaymentMethods = document.getElementById('allowPaymentMethods').value.trim();
                
                // Validate required fields
                if (!paymentAmount || !currencyCode || !refTrx || !successRedirect || !cancelRedirect || !ipnUrl) {
                ...
                // Optional: allow_payment_methods supports CSV string or JSON array
                if (allowPaymentMethods) {
                    try {
                        if (allowPaymentMethods.startsWith('[')) {
                            requestBody.allow_payment_methods = JSON.parse(allowPaymentMethods);
                        } else {
                            requestBody.allow_payment_methods = allowPaymentMethods;
                        }
                    } catch (e) {
                ...
                const requestUrl = new URL('{{ url('/api/v1/initiate-payment') }}');
            ...
            async function testVerifyPayment(environment, apiKey, merchantKey, apiSecret) {
                ...
                const requestUrl = new URL(`{{ url('/api/v1/verify-payment') }}/${encodeURIComponent(transactionId)}`);
                const headers = await buildSignedHeaders(
                    environment,
                    apiKey,
                    merchantKey,
            ...
            function displayResponse(response, responseTime) {
                ...
                if (response.body && response.body.error && response.body.troubleshooting) {
                    ...
                    responseBodyElement.textContent = `API Testing Error\n\n${JSON.stringify(response.body, null, 2)}`;
                } else {
